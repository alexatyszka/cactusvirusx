...
par(mar = c(0, 0, 0, 0))
ggtree(treetempjoined, ladderize=T, show.legend = FALSE, aes(color=group))+
    #host tip labels:
   geom_tiplab(aes(label=host, subset = !is.na(host)), linetype="dotted", align=T, fontsize=3, na.rm=F, offset=0.8, fontface= "bold")+
    #virus name tip labels:
    geom_tiplab(aes(), geom="label",fontsize=3, fill="white", offset=0.1, fontface= "bold",family='Helvetica', align=F, label.size = 0)+
    #newness
   geom_tiplab(aes(label=new),size=10, color="black", align=T, na.rm=F, offset=0.75,vjust=0.8, linetype="blank", fontface=1) +
    #group labels
    scale_color_manual(name="Group", values=c("#BE81EF", "#00A9E7", "#00B9A5", "#3FB13A", "#B39B00", "#E67E67", "#EC6DC4", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7", "#a6cee3","#1f78b4","#b2df8a","#33a02c"))+
  #scale
...
ggsave(filename, width = 70, height = 40, units = "cm")
}
}
