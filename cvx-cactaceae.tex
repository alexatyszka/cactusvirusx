...
to assemble \textit{S. truncata} and GenBank accessed RNA-seq data to reference genome XXXX (Table~S2).

%FIXME: table s2 should describe the coverage that we obtained from the assembly
\begin{comment}
...
Figure inventory:
\begin{itemize}
\item Figure 2. Selection graph as displayed in B's r code

\item Figure 3. (maybe supplemental) density of cp, rdrp, distributions density by virus ptp
\end{itemize}
\begin{subfigures}
\begin{figure}[ht]
\centering
\includegraphics[width=1\linewidth]{figures/tree_circular_collapsed.pdf}
 \begin{NoHyper}
...
\section*{Tables}
\begin{itemize}
\item Table 1. Assembly stats. our taxa, SRAs, assembly stats for positive finds, including unique ID and sample names, including length

\item Table 2. Points under positive selection, how many in each gene
\end{itemize}
\end{comment}
