...
%FIXME decide whether mPTP or bPTP is better for this analysis. Here we prefer ....
...
We summarize these metrics in a table (Table X). %FIXME add table
...
\newpage{}
 \begin{figure}[ht]
 \centering
 \includegraphics[width=0.75\linewidth]{{figures/tree_rect}.pdf}
 \begin{NoHyper}
 \caption{
A maximum likelihood tree of all complete potexvirus genomes (including all five genes) present in this manuscript, including newly assembled viral sequences from both new sequences and publicly accessible sources, and previously described, officially classified viral samples.
Coloration of bars along the right side of the phylogeny represents the delimited species according to each method, 
Newly assembled viral sequences expand the phylogeny by nearly XX\%.
 Official ICTV-determined taxonomy, if present, is listed to the right of the colored bars.
The subset of cactus-infecting potexviruses found in the NCBI SRA (tip labels beginning with SRR) displays narrow sampling across related cohort plants which may represent local viral transmission from plant to plant. 
Newly assembled viral samples within the topmost clade, containing Cactus Virus X samples, (colored in shades of mustard yellow), expand the host range of this clade to include \textit{Schlumbergera truncata}.
UFboot scores are represented on nodes. 
The outgroup clade consists of closely related but non-cactus infecting potexviruses and is not shown on this phylogeny. 
}
 \label{fig:fig1}
 \end{NoHyper}
 \end{figure}
...
%FIXME: AT: Add context for the bars below the plot.
}
...
%Table 1: RNA-seq and assembly stats
\begin{table}[ht]
\centering
