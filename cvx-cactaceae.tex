...
Sequence similarity methods are an alternative method that has been used to delimit viral species.
(%fixme, cite)
...
Viral infections are known to spread to nearly all tissues within a plant (\citealt{Hipper_2013}). %cite
...
Plant immune response s dependent on viral recognition, which may impose selective pressure differentially on viral genes depending on their function.
...
Reproductive tissue is susceptible to viral infection in at least a few cases, for example, Tulip breaking virus.
The presence of virus in reproductive tissue implies that sexual transmission of a virus is possible (\citealt{Kim_2015}).
We recover viruses from pollen and style for \textit{Schlumbergera truncata} {Table~S1, Table~S4}, which is the first reported instance of viral reads on reproductive tissue of this species.
...
In a case where we cannot use the biological species conflict, we find a dilemma. 
The kakapo pipeline allowed us to mine both newly-sequenced data and publicly available SRA data for viral discovery. 
-
-
-Mytcor virus x
