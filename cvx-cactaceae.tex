...
%FIXME: MAFFT settings/options missing. DNA or protein? These paragraphs should be detailed.
...
%B: I went through some files and found several iq-tree files. I assume that the latest one, genbank_kr_consensus_extraction_al_trimmed.fasta.iqtree is the correct one. If the others are not being used, they should be deleted from the shared git archive.
...
%Results: We also implemented an identical analysis for each of five genes, separately, but the results are virtually identical.
Branch support was assessed with ultrafast bootstrap ({\em UFBoot}) \citep{hoang2018}.


% FIXME: this is how we got the sequence for the analyses, which are above. The data acquisition should be placed before the analyses. Move up and in order.
...
Percentage sequence distance was calculated for Coat Protein and RdRp nucleotides and amino acids using SDT version X.X. %FIXME
Sequences with incomplete CP or RdRp sequences were removed post analysis. %post-analysis? If it is after the analyses? If so, it seems to be not used in analyses, which is odd. I think you mean after some of the analyses before others, maybe. Detailed description missing.
...
Expanded phylogenetic trees and individual gene/protein trees are available in the Supplementary Data. %Is it Supplementary Information Appendix or Supplementary Data?
All sequence data is deposited in GenBank under project accession number PRJNXXXXX (https://www.ncbi.nlm.nih.gov/Traces/study/?acc=PRJNXXXXX). %Not all? specifically, it's the schlumbergera data, right? Which ones?
