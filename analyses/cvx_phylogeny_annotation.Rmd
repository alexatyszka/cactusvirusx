...
tip.rename.host = function(tree,dataframe){
   for(i in 1:length(tree$tip.label)){
      ...
      print(dataframe$host[grep(tree$tip.label[i],dataframe$Name)])
      print(dataframe$Organism[grep(tree$tip.label[i],dataframe$Name)])
      tree$tip.label[i]<-dataframe$host[grep(tree$tip.label[i],dataframe$Name)]

   }
tree
}
nj.gg <- read.tree(file="cacti.njtree.nwk")
plot(ladderize(tip.rename.host(nj.gg,cvx.metadata)), type="f",cex=0.65, align.tip.label = TRUE)
```
