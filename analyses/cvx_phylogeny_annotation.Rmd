...
```{r}
par( mar=c(1, 3, 0, 0))
ggtree(nj.gg, ladderize=T, aes(), show.legend = TRUE, size = 1,)  %<+% cvx.metadata +
  #name tip labels:
  geom_tiplab(size=2,aes(label=cvx.metadata$Organism[1:210]), hjust=0, offset=0, align=T, linetype = "dotted")+
  #host tip labels:
  geom_tiplab(size=2,aes(label=cvx.metadata$host[1:210]), align = T, linetype = 'blank', na.rm=TRUE, offset=0.1) +
    geom_tiplab(aes(label=cvx.metadata$country[1:210]), align = T, linetype = 'blank', na.rm=TRUE, offset=0.2) +
    geom_tiplab(aes(label=cvx.metadata$Name[1:210]), align = T, linetype = 'blank', na.rm=TRUE, offset=0.25) +
  #scale
