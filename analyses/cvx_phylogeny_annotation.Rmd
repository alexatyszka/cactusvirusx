...
{	
	for(i in 1:length(solcross.data$pollen_parent))
	{
		ifelse(is.na(match(solcross.data$pollen_parent[i],pathd8.tre$tip.label)) | is.na(match(solcross.data$seed_parent[i],
		pathd8.tre$tip.label)),d[i]<-NA,
		d[i]<-cophenetic.phylo(pathd8.tre)[solcross.data$pollen_parent[i],solcross.data$seed_parent[i]])
	}
	return(d)
}
```
