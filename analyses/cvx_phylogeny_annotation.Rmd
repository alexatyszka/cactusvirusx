...
write(x=seq.metadatwa.csv, file='seq.metadata.csv')
```
...
Edit host names to add placeholder for unknown host, and root the tree (any root will do for now, as we can reroot it later):
...
#cacti.njtree <- root(cacti.nj, outgroup=c(7), resolve.root=T)
...
```{r}
par( mar=c(1, 3, 0, 0))
ggtree(nj.gg, ladderize=F, aes(), show.legend = TRUE,)  %<+% cvx.metadata +
  #name tip labels:
  geom_tiplab(aes(label=cvx.metadata$Organism[1:211]), hjust=0, offset=0, align=T, linetype = "dotted")+
  #host tip labels:
  geom_tiplab(aes(label=cvx.metadata$host[1:211]), align = T, linetype = 'blank', na.rm=TRUE, offset=0.1) +
    geom_tiplab(aes(label=cvx.metadata$country[1:211]), align = T, linetype = 'blank', na.rm=TRUE, offset=0.2) +
    geom_tiplab(aes(label=cvx.metadata$Name[1:211]), align = T, linetype = 'blank', na.rm=TRUE, offset=0.25) +
  #scale
