...
rdrp <- read.csv('/Users/alexa/Desktop/cactusvirusx/data/data_v3/percent_seq_id/1_RdRp_perseq.csv', stringsAsFactors = FALSE, row.names = 1)
cp <- read.csv('/Users/alexa/Desktop/cactusvirusx/data/data_v3/percent_seq_id/1_RdRp_perseq.csv', stringsAsFactors = FALSE, row.names = 1)
...
cvafirst <- cva[1]
cvalast <-tail(cva, n=1)

rdrpmaxcva <-c(max(rdrp[cvafirst:cvalast,cvafirst:cvalast]))
rdrpavgcva <- c(mean(rowMeans(rdrp[cvafirst:cvalast,cvafirst:cvalast])))

```
Fetch values for table.
```{r}
lnames <- rep(c("Full sequence", "Coat Protein", "RdRp"),2)
...
```{r}
sumstats <- list(lnames, lmax, lavg)
```
